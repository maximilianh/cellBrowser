...
exportData = function(object, dataName, outputDir, markerField, clusterField, use.scaleData){
  ...
  writeMM(counts, file = 'counts_exprMat.mtx')
  ...
  gzip('counts_exprMat.mtx', overwrite = T, remove = T)
  ...
  colnames(cpm) = NULL
  rownames(cpm) = NULL
  writeMM(cpm, file = 'data_exprMat.mtx')
  ...
  if(use.scaleData){
    ...
    writeMM(zscore, file = 'scale_exprMat.mtx')
    gzip('scale_exprMat.mtx', overwrite = T, remove = T)
  }
...
coords=%s
  ...
  if(use.scaleData){
  matrices.string = paste0("matrices=[{'label':'counts','fileName':'counts_exprMat.mtx.gz'},\n",
                            "{'label':'data','fileName':'data_exprMat.mtx.gz'},\n",
                            "{'label':'scale','fileName':'scale_exprMat.mtx.gz'}]" )
  } else {
    matrices.string = paste0("matrices=[{'label':'counts','fileName':'counts_exprMat.mtx.gz'},\n",
                             "{'label':'data','fileName':'data_exprMat.mtx.gz'}]")
  }
